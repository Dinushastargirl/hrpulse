...
export default function Layout({ children }: { children: React.ReactNode }) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        if (docSnap.exists()) {
          ...
          setUser(userData);
          if (userData.mustResetPassword && location.pathname !== '/reset-password') {
  ...
  const navItems = [
    { label: 'Dashboard', path: '/dashboard', icon: LayoutDashboard, roles: ['employee', 'admin'] },
    { label: 'Attendance', path: '/attendance', icon: Clock, roles: ['employee'] },
    { label: 'Tasks', path: '/tasks', icon: CheckSquare, roles: ['employee'] },
    { label: 'Calendar', path: '/calendar', icon: Calendar, roles: ['employee', 'admin'] },
    { label: 'Employees', path: '/admin/employees', icon: Users, roles: ['admin'] },
    { label: 'Leave Requests', path: '/admin/leaves', icon: Briefcase, roles: ['admin'] },
    { label: 'Profile', path: '/profile', icon: UserCircle, roles: ['employee', 'admin'] },
  ];
